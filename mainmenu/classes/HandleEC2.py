...
from .constants.SSHActions import SSHActions
...
class HandleEC2(object):
    ...
    def set_securitygroup_ids(self, securitygroup_ids):
        self.securitygroup_ids = securitygroup_ids
        return
    ...
    def handle_ec2_action(self , action:str):
        ...
        if action == EC2Actions.create.name:
            ...
            try:
                if self.securitygroup_ids is None:
...
# 
    ...
    def wakeup_ec2(self, wait_time:int = 90,delay:int = 1 ):
        ...
        if ec2_state == EC2Status.stopped.name:
            ...
            self.handle_ec2_action(action=EC2Action.start.name)
    ...
    # def create_ec2_instance_from_template(self,
    #     securitygroup_ids:list = None
    # ):
    #     logging.info("Create new EC2 instances")
    #     try:
    #         ec2_instance_id = create_instance(     
    ...
    #     except Exception as e:
    #         raise Exception(f"Create ec2 instance failed {e}")
    # def handle_ssh_command(self, action, pem_file:str,ssh_command:str):
    #     if action is None:
    #         raise ValueError("ssh action is None")

    #     remote_base_path = f"/home/{self.login_user}/gismo-cloud-deploy/gismoclouddeploy/services/"
    #     full_ssh_command = f"cd {remote_base_path} \n source ./venv/bin/activate \n {ssh_command} "
        
    #     if action == SSHActions.debug.name:
    ...
    #     elif action == SSHActions.run_files.name:
    #         logging.info("run files")
...
def create_ec2_object_from_existing_yaml(
