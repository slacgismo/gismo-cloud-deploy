...
class AWSServices(object):
    ...
    def handle_ec2_action(
        ...
        if action == EC2Actions.start.name:
            if ec2_instance_id is not None:
                ...
                instance = check_if_ec2_ready_for_ssh(instance_id=ec2_instance_id, wait_time=self._ssh_total_wait_time, delay=self._ssh_wait_time_interval, pem_location=self.get_pem_file_full_path_name(),user_name=login_user)
    ...
    def generate_eks_config_and_export(self, eks_config_yaml_dcit:str):
    ...
    def check_eks_exist(self,
        ...
        privkey = paramiko.RSAKey.from_private_key_file(self.get_pem_file_full_path_name())
        ssh.connect(p2_instance.public_dns_name,username=login_user,pkey=privkey)
        print('started...')
...
def add_public_ip_to_sshconfig(
        ...
        for host in  c.hosts():
            if host == public_ip:
                logging.info(f"{public_ip} exist in .ssh/config")
                return
        c.add(public_ip, 
            Hostname=public_ip, 
            User=login_user,
