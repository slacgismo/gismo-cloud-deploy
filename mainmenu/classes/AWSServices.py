...
from .utilities.helper import(
    get_pem_file_full_path_name
    
)

from .utilities.update_sshconfig import add_public_ip_to_sshconfig,delete_public_ip_to_sshconfig
...
class AWSServices(object):
    ...
    def handle_aws_actions(self, action:AWSActions):
        ...
        if action == AWSActions.create_securitygroup.name:
            ...
            sg_id = get_security_group_id_with_name(ec2_client=self._ec2_client, group_name=self._securitygroup_name)
            if sg_id is None:
                ...
                logging.info(f"Found security groupd with name: {self._securitygroup_name}  ids: {self._securitygroup_ids}")
                self._securitygroup_ids = [sg_id]
            ...
            sg_id = get_security_group_id_with_name(
                ...
                group_name=self._securitygroup_name)
            if sg_id is None:
                logging.info(f"No security group name :{self._securitygroup_name} found")
            else:
                logging.info(f"Found security groupd with name: {self._securitygroup_name}  id: {sg_id}")
                self._securitygroup_ids = [sg_id]
            ...
            try:
                ...
                if not check_keypair_exist(
                    ec2_client=self._ec2_client, 
                    keypair_anme=self._keypair_name
                    ):
            ...
            if not check_keypair_exist(ec2_client=self._ec2_client, keypair_anme=self._keypair_name):
                ...
                delete_key_pair(ec2_client=self._ec2_client, key_name=self._keypair_name)
                local_pem  = get_pem_file_full_path_name(self.local_pem_path,self._keypair_name)
                if exists(local_pem):
                    os.remove(local_pem)
    ...
    def generate_eks_config_and_export(self, eks_config_yaml_dcit:str, export_file:str):
